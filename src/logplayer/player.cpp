    ...
    m_legacyVision(NULL)
{
    ...
    m_referee = new Network(QHostAddress("127.0.0.1"), 0, 10003);
    m_referee->connect();

    // create vision socket
    Q_ASSERT(m_vision == NULL);
    m_vision = new Network(QHostAddress("127.0.0.1"), 0, 10006);
    m_vision->connect();

    // create legacy vision socket
    Q_ASSERT(m_legacyVision == NULL);
    m_legacyVision = new Network(QHostAddress("127.0.0.1"), 0, 10005);
...
bool Player::load(const QString& filename, int& maxFrame, double& duration) {
    std::cout << "Player::load() filename=" << filename.toStdString() << " maxFrame=" << maxFrame << " duration=" << duration << std::endl;
