...
LogFile::LogFile(const QString& filename, bool compressed, int formatVersion) :
    ...
    m_compressor(NULL)
{
    std::cout << "New LogFile() filename=" << filename.toStdString() << " compressed=" << compressed << " formatversion=" << formatVersion << std::endl;
...
{
//    std::cout << "LogFile::openRead()" << std::endl;
...
{
    std::cout << "LogFile::openWrite()" << std::endl;
    close();
...
{
//    std::cout << "LogFile::readMessage()" << std::endl;
