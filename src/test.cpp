...
//    std::string filename = "../../2018-06-19_16-35_RoboDragons-vs-RoboTeam_Twente.log";
    ...
    double total_duration_game = std::get<0>(tracker.getInfo().timeline.back()) - std::get<0>(tracker.getInfo().timeline.front());
    std::string started_at;
    std::string stopped_at;
    {
        char buff[32];
        time_t time = (time_t)std::get<0>(tracker.getInfo().timeline.front());
        strftime(buff, 20, "%Y-%m-%d %H:%M:%S", localtime(&time));
        started_at = std::string(buff);

        time = (time_t)std::get<0>(tracker.getInfo().timeline.back());
        strftime(buff, 20, "%Y-%m-%d %H:%M:%S", localtime(&time));
        stopped_at = std::string(buff);
    }
    {
        double previous = std::get<0>(tracker.getInfo().timeline.back());
        for (unsigned long i = tracker.getInfo().timeline.size() - 1; 0 < i; i--) {
            auto tup = tracker.getInfo().timeline.at(i);
    ...
    tracker.tick();
    double timestamp_first = tracker.get().timestamp;
    int iStates = 0;
    ...
    const double gameDuration = tracker.get().timestamp - timestamp_first;
    ...
    std::cout << "Packets processed= " << reader.packetsRead << std::endl;
