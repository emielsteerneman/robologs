...
Reader::~Reader() {
    std::cout << "[Reader] Destructing new reader" << std::endl;
}
...
int Reader::next(){
    ...
    in.read((char*) &this->dataHeader, sizeof(this->dataHeader));
    // Log data is stored big endian, convert to host byte order
    this->dataHeader.timestamp = be64toh(this->dataHeader.timestamp);
    this->dataHeader.messageType = be32toh(this->dataHeader.messageType);
    this->dataHeader.messageSize = be32toh(this->dataHeader.messageSize);

    this->data = new char[this->dataHeader.messageSize];
    in.read(this->data, this->dataHeader.messageSize);

    return this->dataHeader.messageType;
}
...
int main(int argc, char *argv[])
{
    ...
    std::cout << "Type detected: "  << std::to_string(type) << std::endl;
